...
3. 在 `目前履歷版本` 選擇要編輯的版本；下方表單會切換成該版本的內容。
4. 使用 `新增履歷` 複製目前版本建立新履歷，或使用 `刪除履歷` 移除目前版本；至少會保留一份履歷。
5. 使用 `下載 JSON` 匯出目前版本的 `resume.json`，或使用 `複製 JSON` 取得目前版本內容。

編輯器會把多份履歷版本暫存在目前瀏覽器的 `localStorage`，重新整理頁面後仍可繼續編輯。若要帶入外部版本，先切換到目標版本，再使用 `匯入 JSON` 選擇本機的 `resume.json`；匯入內容會覆蓋目前版本。

站內完整版履歷會優先讀取這組本機履歷版本，因此在編輯器修改後點擊 `查看履歷`，可以直接預覽並切換最新版本。
